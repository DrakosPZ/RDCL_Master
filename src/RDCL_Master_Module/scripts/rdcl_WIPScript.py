...
class PW_AOVMaster(object):
    ...
    def presetSave_BTNAction(self, *args):
        AOVConstructionObject = self.unpackAOVTulipforList(self.ListOfAOVS)
        dialogResult = cmds.fileDialog2(bbo = 2, cap = 'Save Template as', fm = 0)
    ...
    def presetLoad_BTNAction(self, *args):
        ...
        print('TBD WARNING HAS YET TO BE IMPLEMENTED')
...
def testLoading():
    ...
    if len(dialogResult) > 0:
        ...
        python_obj = json.loads(fileOutput)
        pythonSplit = python_obj[1:-1].split(', ')
        print(pythonSplit);
